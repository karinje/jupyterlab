...
def build_cell_output_update(cell_index: int, outputs: list, execution_count: int = None):
    """
...
async def execute_code_and_get_outputs(code: str, kernel_id: str, token: str, server_url: str):
    ...
    try:
        async with websockets.connect(ws_url) as websocket:
            print(f"✅ Connected to kernel WebSocket")
            ...
            while True:
                try:
                    ...
                    if msg_type == "stream":
                        ...
                        print(f"✅ Execution completed: status={status}, count={execution_count}")
                        break
...
async def get_current_token_and_kernel():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get("http://127.0.0.1:8890/api/kernels", headers=headers) as resp:
            if resp.status == 200:
                ...
                if kernels:
                    ...
                    async with session.post("http://127.0.0.1:8890/api/kernels", 
                                          headers=headers, 
                                          json={"name": "python3"}) as resp:
...
async def test_multiple_cell_targeting():
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(f"{server_url}/api/kernels", 
                              headers=headers, 
                              json={"name": "python3"}) as resp:
            if resp.status == 201:
                kernel_data = await resp.json()
...
'''
    ...
    outputs_a, exec_count_a = await execute_code_and_get_outputs(code_a, kernel_id, token, server_url)
    
    # Execute CELL B (main computation) 
    print("🔸 Executing CELL B (computation)...")
    outputs_b, exec_count_b = await execute_code_and_get_outputs(code_b, kernel_id, token, server_url)
    
    # Put CELL B's outputs into CELL C
    print("🔸 Putting CELL B outputs into CELL C...")
    if outputs_b:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{server_url}/api/contents/{path}", headers=headers) as resp:
                notebook_data = await resp.json()
                cells = notebook_data['content']['cells']
            ...
            async with session.post(f"{server_url}/api/agent/notebook/update_outputs",
                                  headers=headers, json=update_data) as resp:
                if resp.status == 200:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f"{server_url}/api/contents/{path}", headers=headers) as resp:
            ...
            for i, cell in enumerate(cells):
                ...
                print(f"  Cell {i}: ID={cell_id_found[:8]}... outputs={has_outputs} exec_count={exec_count}")
                print(f"         Source: {source_preview}...")
                
                # Verify our specific cells
                if cell_id_found == cell_id:
                    print(f"    ✅ Original cell: {len(cell.get('outputs', []))} outputs")
                elif cell_id_found == cell_a_id:
                    print(f"    📝 CELL A (storage): {len(cell.get('outputs', []))} outputs")
                elif cell_id_found == cell_b_id:
                    print(f"    🔧 CELL B (executed): {len(cell.get('outputs', []))} outputs")  
                elif cell_id_found == cell_c_id:
                    print(f"    📤 CELL C (received outputs): {len(cell.get('outputs', []))} outputs")
            
            print(f"\n🎯 Agent Control Test Results:")
            print(f"   - CELL A: Contains setup code")
            print(f"   - CELL B: Contains computation code") 
            print(f"   - CELL C: Should have CELL B's execution outputs")
            ...
            for cell in cells:
                if cell.get('id') == cell_c_id:
                    ...
                    if c_outputs:
                        print(f"   ✅ SUCCESS: CELL C has {len(c_outputs)} outputs from CELL B execution!")
                        for j, output in enumerate(c_outputs):
                            print(f"      Output {j+1}: {output.get('output_type', 'unknown')}")
                    else:
    ...
    return {
        ...
        "cell_c": {"cell_id": cell_c_id, "purpose": "output_destination", "outputs_count": len(outputs_b)},
