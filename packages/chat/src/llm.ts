...
export class OpenAIProvider extends BaseLLMProvider {
  ...
  async sendMessage(message: string, context?: any): Promise<string> {
    ...
    if (!this._apiKey) {
      ...
      return "I'm a JupyterLab assistant, but I need an OpenAI API key to be configured to provide intelligent responses. You can set this in the settings. For now, I can confirm that I received your message: " + message;
    }
...
export class ClaudeProvider extends BaseLLMProvider {
  ...
  async sendMessage(message: string, context?: any): Promise<string> {
    ...
    if (!content || typeof content.text !== 'string') {
      throw new Error('Invalid response from Claude API: invalid content format');
    }
