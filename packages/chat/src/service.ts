    ...
    try {
      ...
      if (activeThreadId && data.threads && data.threads[activeThreadId]) {
        ...
        for (const msg of threadMessages) {
          ...
          const isRespondToUserCall =
            messageType === 'tool_call' &&
            msg.tool_calls &&
            msg.tool_calls.length > 0 &&
            (msg.tool_calls[0].function?.name === 'RespondToUser' ||
              msg.tool_calls[0].name === 'RespondToUser');
          ...
          if (!messageType && msg.role === 'tool') {
            console.log(
              `🔄 Filtering out legacy tool message (no metadata): ${msg.content?.substring(
                0,
                50
              )}`
            );
          ...
          if (!messageType && msg.content?.startsWith('responded: intent=')) {
            console.log(
              `🔄 Filtering out legacy RespondToUser result: ${msg.content?.substring(
                0,
                50
              )}`
            );
  ...
  async switchThread(threadId: string): Promise<void> {
    try {
      ...
      for (const msg of selectedThread.messages || []) {
        ...
        console.log(
          `🔍 [switchThread] Processing message: role=${
            msg.role
          }, messageType=${messageType}, content=${msg.content?.substring(
            0,
            50
          )}`
        );
        ...
        console.log(
          `🔍 [switchThread] isRespondToUserCall=${isRespondToUserCall}, has tool_calls=${!!msg.tool_calls}`
        );
        if (msg.tool_calls) {
          console.log(
            `🔍 [switchThread] tool_calls structure:`,
            JSON.stringify(msg.tool_calls).substring(0, 200)
          );
        }
        ...
        if (!messageType && msg.role === 'tool') {
          console.log(
            `🔄 [switchThread] Filtering out legacy tool message: ${msg.content?.substring(
              0,
              50
            )}`
          );
        ...
        if (!messageType && msg.content?.startsWith('responded: intent=')) {
          console.log(
            `🔄 [switchThread] Filtering out legacy RespondToUser: ${msg.content?.substring(
              0,
              50
            )}`
          );
