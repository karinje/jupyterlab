...
export class CellManager implements ICellManager {
  ...
  setCellContent(cellIndex: number, content: string): void {
    console.log(
      `🔧 CellManager.setCellContent called: index=${cellIndex}, content="${content}"`
    );
    ...
    if (cell.value && cell.value.text !== undefined) {
      ...
      console.error(
        `🔧 Cannot set content for cell ${cellIndex} - no accessible content API`
      );
      throw new Error(
        `Cannot set content for cell ${cellIndex} - no accessible content API`
      );
  ...
  insertCell(
    ...
    console.log(
      `🔧 CellManager.insertCell called: index=${cellIndex}, content="${content}", type=${cellType}`
    );
    ...
    console.log(
      '📝 Available contentFactory methods:',
      Object.getOwnPropertyNames(notebook.model.contentFactory)
    );
    ...
    if (cellModel.sharedModel) {
      ...
      console.log(
        '📝 Content set via sharedModel.source:',
        cellModel.sharedModel.source
      );
    ...
    console.log(
      `📝 Inserting at index ${insertIndex} (requested: ${cellIndex}, max: ${cells.length})`
    );
      ...
      for (let i = 0; i < cells.length; i++) {
        ...
        if (cell) {
          ...
          if (cell.value && cell.value.text !== undefined) {
            ...
            cell.sharedModel &&
            cell.sharedModel.source !== undefined
          ) {
  ...
  getCurrentCell(): { content: string; type: string; index: number } | null {
    ...
    if (cell.model.value && cell.model.value.text !== undefined) {
      ...
      cell.model.sharedModel &&
      cell.model.sharedModel.source !== undefined
    ) {
  ...
  scrollToCell(cellIndex: number): void {
    ...
    try {
      if (notebook.scrollToItem) {
        ...
        console.warn(
          '[CellManager] scrollToItem method not available on notebook'
        );
      }
    } catch (error) {
