...
class MCPClient:
    ...
    async def connect(self) -> bool:
        ...
        for server_name, config in self.servers_config.items():
            try:
                ...
                tool_names = (
                    [t.name for t in tools_result.tools] if tools_result.tools else []
                )
                logger.info(
                    f"✅ Connected to {server_name}. Available tools: {tool_names}"
                )
    ...
    async def get_schema(self, server_name: str, table_name: str) -> str:
        ...
        return await self.call_tool(
            server_name, "describe_table", {"table_name": table_name}
        )
